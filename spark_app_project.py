...
dataStream = ssc.socketTextStream("twitterProject", 9009) # Now this is where we connect to our Twitter app and get the tweets.
...
tags = ['stress', 'depression', 'depressed', 'anxious', 'anxiety', 'lonely']
...
def process_interval(time, rdd):
    ...
    try:
        ...
        # We recreate the file from scratch to add the results (and so to not overlap with past values.)
        # print it nicely
        for tag in top10:
            print('{}: {}'.format(tag[0], tag[1]))
             # We also print the output of the Apache stream to a file.
